...
class LearnersEnsemble(object):
    ...
    def evaluate_iterator(self, iterator):
        ...
        with torch.no_grad():
            ...
            #DEBUG
            # print("in learners_ensemble 250 line")
    ...
    def get_output(self, iterator):
        ...
        with torch.no_grad():
            for i,(x, y, _) in enumerate(iterator):
                ...
                if i == 0:  
                    output = np.array(y_pred.cpu())
                else:
                    output= np.concatenate((output, y_pred.cpu()), axis=0)

            for learner in self.learners:
                learner.model.to('cpu')
            return output

    def get_first_model_output_prototype(self, iterator):
        """
        ...
        with torch.no_grad():
            for i, (x, y, _) in enumerate(iterator):
                ...
                output = self.learners[0].model(x)
            ...
            average_features = np.zeros((class_num, features.shape[1]))

            for i in range(class_num):
                class_features = features[labels == i]
                if class_features.size > 0:
                    average_features[i] = np.mean(class_features, axis=0)
    ...
    def get_first_model_output_and_labels(self, iterator):
        """
        :param iterator: yields x, y, indices
        :type iterator: torch.utils.data.DataLoader
        :return: global_loss, global_acc
    ...
    def get_all_models_loss_and_accuracy(self,iterator):
            ...
            for learner_id, learner in enumerate(self.learners):
                ...
                for (x, y, _) in iterator:
                    ...
                    if self.is_binary_classification or isinstance(self.learners[0].criterion, BinaryClassifierLoss):
                            ...
                            y_pred = torch.sigmoid(learner.model(x))
                    else:
                        
                            y_pred = F.softmax(learner.model(x), dim=1)
                    if self.is_binary_classification or isinstance(self.learners[0].criterion, BinaryClassifierLoss):
                        
                        y = y.type(torch.long)
                        # y = F.one_hot(y, self.learners[0].criterion.class_number) + 0.0
                        # y = y.type(torch.float32).unsqueeze(1)
                        loss += criterion(y_pred,F.one_hot(y, self.learners[0].criterion.class_number) + 0.0).sum().item()
                        y_pred = torch.logit(y_pred, eps=1e-10)
                    else:
                        
                        y = y.type(torch.long)
                        loss += criterion(torch.log(y_pred), y).sum().item()
                    
                    metric += self.metric(y_pred, y).item()
                losses.append(loss/n_samples)
                accuracies.append(metric/n_samples)
        for learner in self.learners:
            learner.model.to('cpu')
    ...
    def evaluate_iterator(self, iterator):
        ...
        with torch.no_grad():
            for (x, y, _) in iterator:
                ...
                p_k_x = self.predict_gmm(x).sum(dim=1)  
