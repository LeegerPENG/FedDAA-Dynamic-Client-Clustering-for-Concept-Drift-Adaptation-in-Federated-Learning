...
class Client(object):
    ...
    def write_logs(self):
        ...
        if self.tune_locally: 
            ...
            #DEBUG
            # print("in client 280 line")
            # print(f"Number of test samples: {len(self.test_iterator.dataset)}")
    ...
    def set_data_iterator(self,train_iterator, val_iterator, test_iterator, client_type, feature_type):
        self.train_iterator = train_iterator
        self.val_iterator = val_iterator
        self.test_iterator = test_iterator
        self.data_type = client_type
        self.feature_types = feature_type
    def set_current_data_type(self, client_type, feature_type):
        self.data_type = client_type
        self.feature_types = feature_type
...
class FedRC_Concept_Drift(Client): 
    ...
    def __init__(
            ...
            logger,
            local_steps,
            last_train_iterator=None, 
            last_val_iterator=None,
            last_test_iterator=None,
            tune_locally=False,
            data_type=0,
            feature_types=None,
            class_number=10,
        ...
        self.last_data_type = 0
        self.last_feature_types = None
        self.concept_shift_flag = 0 

        
        self.current_train_iterator = copy.deepcopy(train_iterator)
        self.current_val_iterator = copy.deepcopy(val_iterator)
        self.current_test_iterator = copy.deepcopy(test_iterator)
        #DEBUG
        # print("creat FedRC concept drift, in client.py 466 line")
    ...
    def set_last_data_iterator(self):
        self.last_train_iterator = copy.deepcopy(self.current_train_iterator)
        self.last_val_iterator = copy.deepcopy(self.current_val_iterator)
        self.last_test_iterator = copy.deepcopy(self.current_test_iterator)
        self.last_data_type = copy.deepcopy(self.data_type) 
        self.last_feature_types = copy.deepcopy(self.feature_types)

    def get_output(self):
        
        output_last_time = self.learners_ensemble.get_output(self.last_val_iterator)
        output_current_time = self.learners_ensemble.get_output(self.current_val_iterator)
        output = np.concatenate((output_last_time, output_current_time), axis=0)
        return output

    def get_last_data_all_models_loss_and_accuracy(self):
        
        losses,accuracies = self.learners_ensemble.get_all_models_loss_and_accuracy(self.last_val_iterator)
        return losses,accuracies

    def get_current_data_all_models_loss_and_accuracy(self):
    ...
    def get_real_shift_label(self):
        ...
        if self.data_type in [2,4] and self.last_data_type in [2,4]:
            return 0
        elif self.data_type in [3,5] and self.last_data_type in [3,5]:
            return 0
        elif self.data_type in [0,1] and self.last_data_type in [0,1]:
            return 0
        else:
            return 1

    def set_concept_shift_flag(self,concept_shift_flag):
        self.concept_shift_flag = concept_shift_flag

    def set_current_data_iterator(self,train_iterator, val_iterator, test_iterator):
        """
        ...
        """
        self.current_train_iterator = train_iterator
        self.current_val_iterator = val_iterator
        self.current_test_iterator = test_iterator

    def get_merge_last_and_current_train_iterators(self):
        """
        ...
        train_datasets = [copy.deepcopy(self.last_train_iterator.dataset),copy.deepcopy(self.current_train_iterator.dataset)]
        val_datasets = [copy.deepcopy(self.last_val_iterator.dataset),copy.deepcopy(self.current_val_iterator.dataset)]
        test_datasets = [copy.deepcopy(self.last_test_iterator.dataset),copy.deepcopy(self.current_test_iterator.dataset)]
        ...
        merged_train_iterator = DataLoader(merged_train_dataset, batch_size=self.current_train_iterator.batch_size, shuffle=True,
                                    drop_last=True)
        merged_val_iterator = DataLoader(merged_val_dataset, batch_size=self.current_val_iterator.batch_size, shuffle=True,
                                    drop_last=False)
        merged_test_iterator = DataLoader(merged_test_dataset, batch_size=self.current_test_iterator.batch_size,
                                        shuffle=True,
                                        drop_last=False)


        # merged_train_iterator = chain(self.last_train_iterator,self.current_train_iterator)
        # merged_val_iterator = chain(self.last_val_iterator,self.current_val_iterator)
        # merged_test_iterator = chain(self.last_test_iterator,self.current_test_iterator)
    ...
    def rotate_get_merge_last_and_current_train_iterators(self,rotate_degrees):
        """
        ...
        # datasets = [iterator.dataset for iterator in train_iterators]
        train_datasets = [copy.deepcopy(self.last_train_iterator.dataset),copy.deepcopy(self.current_train_iterator.dataset)]
        val_datasets = [copy.deepcopy(self.last_val_iterator.dataset),copy.deepcopy(self.current_val_iterator.dataset)]
        test_datasets = [copy.deepcopy(self.last_test_iterator.dataset),copy.deepcopy(self.current_test_iterator.dataset)]

        merged_train_dataset = RotateMergedDataset(train_datasets, rotate_degrees=rotate_degrees)
        merged_val_dataset = RotateMergedDataset(val_datasets, rotate_degrees=rotate_degrees)
        merged_test_dataset = RotateMergedDataset(test_datasets, rotate_degrees=rotate_degrees)
        #
    ...
    def update_train_iterator_and_other_attributes(self,merged_train_iterator, merged_val_iterator, merged_test_iterator):
        ...
        self.train_iterator = copy.deepcopy(merged_train_iterator)
        self.val_iterator = copy.deepcopy(merged_val_iterator)
        self.test_iterator = copy.deepcopy(merged_test_iterator)
        ...
        self.feature_types = [0] * (self.n_train_samples+self.n_test_samples)
        ...
        # self.labels_weights = torch.ones(self.n_learners, self.n_train_samples * 80) / self.n_learners
        self.labels_weights = torch.ones(self.n_learners, self.n_train_samples) / self.n_learners
        self.labels_mask = torch.zeros(self.class_number, self.n_train_samples)
...
class FedRC_Concept_Drift_version2(Client): 
    '''
    for 每个time slot都初始化clients
    '''
    ...
    def __init__(
        ...
        self.last_val_iterator = copy.deepcopy(last_val_iterator)
        self.last_test_iterator = copy.deepcopy(last_test_iterator)
        self.last_data_type = copy.deepcopy(last_data_type)
        self.last_feature_types = copy.deepcopy(last_feature_types)
        self.concept_shift_flag = 0 
    ...
    def set_last_data_iterator(self):
        ...
        self.last_test_iterator = copy.deepcopy(self.current_test_iterator)
        self.last_data_type = copy.deepcopy(self.data_type) 
        self.feature_types = copy.deepcopy(self.feature_types)
    ...
    def get_current_val_iterator_output_prototype(self):
        output_prototype = self.learners_ensemble.get_first_model_output_prototype(self.current_val_iterator)
        return output_prototype

    def get_last_val_iterator_output_prototype(self):
        output_prototype = self.learners_ensemble.get_first_model_output_prototype(self.last_val_iterator)
        return output_prototype

    def get_last_val_iterator_output_and_labels(self):
        """
        For  CDD Detection method
        Returns
        -------

        """
        output, labels = self.learners_ensemble.get_first_model_output_and_labels(self.last_val_iterator)
        return output,labels

    def get_current_val_iterator_output_and_labels(self):
        """
        For  CDD Detection method
        Returns
        -------
        ...
        """
        output, labels = self.learners_ensemble.get_first_model_output_and_labels(self.current_val_iterator)
        return output, labels
    ...
    def get_real_shift_label(self):
        ...
        if self.data_type == self.last_data_type:
            return 0
        else:
            return 1
    ...
    def rotate_120_get_merge_last_and_current_train_iterators(self,rotate_degrees):
    ...
    def rotate_120_get_merge_last_and_current_train_iterators_fmnist(self,rotate_degrees):

        # datasets = [iterator.dataset for iterator in train_iterators]
        train_datasets = [copy.deepcopy(self.last_train_iterator.dataset),copy.deepcopy(self.current_train_iterator.dataset)]
        val_datasets = [copy.deepcopy(self.last_val_iterator.dataset),copy.deepcopy(self.current_val_iterator.dataset)]
        test_datasets = [copy.deepcopy(self.last_test_iterator.dataset),copy.deepcopy(self.current_test_iterator.dataset)]
        merged_train_dataset = Rotate120MergedDatasetFmnist(train_datasets, rotate_degrees=rotate_degrees)
        merged_val_dataset = Rotate120MergedDatasetFmnist(val_datasets, rotate_degrees=rotate_degrees)
        merged_test_dataset = Rotate120MergedDatasetFmnist(test_datasets, rotate_degrees=rotate_degrees)
    ...
    def set_learning_rate(self, new_learning_rate):
        for learner in self.learners_ensemble:
            learner.set_learning_rate(new_learning_rate)
