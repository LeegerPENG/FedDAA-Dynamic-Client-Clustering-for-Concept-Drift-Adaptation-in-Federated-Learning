...
def get_cifar10C_loaders_for_concept_drift(root_path, batch_size, is_validation, test=False, test_num = 3, train_num=100):
    train_iterators, val_iterators, test_iterators, client_types, client_features = [], [], [], [], []
    if test: 
        for i in range(1, test_num + 1):
            task_data_path = os.path.join(root_path, 'test-{}.pkl'.format(i)) 
    ...
    indices = torch.randperm(train_num) 
    for i in indices: 
        task_data_path = os.path.join(root_path, '{}.pkl'.format(i))
        with open(task_data_path, "rb") as f:
            client = pickle.load(f) 
...
def get_cifar10C_loaders_for_store_history(root_path, batch_size, is_validation,data_indexes=None, test=False, test_num = 3, train_num=300):
    train_iterators, val_iterators, test_iterators, client_types, client_features = [], [], [], [], []
    if test: 
        for i in range(1, test_num + 1):
            task_data_path = os.path.join(root_path, 'test-{}.pkl'.format(i)) 
...
def get_cifar10C_loaders_for_store_history_rotate_images(root_path, batch_size, is_validation,rotate_degrees,data_indexes=None, test=False, test_num = 3, train_num=300):
    ...
    if test: 
        for i in range(1, test_num + 1):
            ...
            train_dataset = SubCIFAR10C(train_indices, cifar10_data=client['images'], cifar10_targets=client['labels'],transform=transform) #SubCIFAR10C在datasets.py里面
            val_dataset = SubCIFAR10C(val_indices, cifar10_data=client['images'], cifar10_targets=client['labels'],transform=transform)
            test_dataset = SubCIFAR10C(test_indices, cifar10_data=client['images'], cifar10_targets=client['labels'],transform=transform)
            train_iterator = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
            val_iterator = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, drop_last=False)
            test_iterator = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, drop_last=False)
...
def get_fmnistC_loaders_for_store_history_rotate_images(root_path, batch_size, is_validation,rotate_degrees,data_indexes=None, test=False, test_num = 3, train_num=300):
    """
    Parameters
    ----------
    root_path
    ...
    if test: 
        for i in range(1, test_num + 1):
            ...
            train_dataset = SubFEMNISTC(train_indices, cifar10_data=client['images'], cifar10_targets=client['labels'],transform=transform) 
            val_dataset = SubFEMNISTC(val_indices, cifar10_data=client['images'], cifar10_targets=client['labels'],transform=transform)
            test_dataset = SubFEMNISTC(test_indices, cifar10_data=client['images'], cifar10_targets=client['labels'],transform=transform)
            train_iterator = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
            val_iterator = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, drop_last=False)
            test_iterator = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, drop_last=False)
...
def get_client_for_store_history_version_2(
        client_type,
        learners_ensemble,
        q,
        train_iterator,
    ...
    if client_type == "mixture":
        ...
        return FedRC_Concept_Drift_version2(
            learners_ensemble=learners_ensemble,
            train_iterator=train_iterator,
            val_iterator=val_iterator,
            test_iterator=test_iterator,
