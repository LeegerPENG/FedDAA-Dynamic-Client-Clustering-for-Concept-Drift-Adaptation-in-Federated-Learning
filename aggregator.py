...
class CentralizedAggregator(Aggregator):
    ...
    def get_global_learners_ensemble(self):
        return self.global_learners_ensemble

    def set_learning_rate(self, new_learning_rate):
        for client in self.clients:
            client.set_learning_rate(new_learning_rate)

    def mix_for_FLASH(self,m,v,d,flag, diverse=True):
        ...
        for learner_id, learner in enumerate(self.global_learners_ensemble): 
            learners = [client.learners_ensemble[learner_id] for client in
                        self.sampled_clients]  

            new_m, new_v, new_d = average_learners_for_FLASH(learners, learner, m,v,d,flag)

        if self.split:

            base_learners = [client.learners_ensemble.base_learner for client in self.sampled_clients]
            average_learners(base_learners, self.global_learners_ensemble.base_learner,
                             weights=torch.ones((len(self.sampled_clients),)) / len(self.sampled_clients))

        if self.domain_disc:

            disc_learners = [client.learners_ensemble.domain_disc_learner for client in self.sampled_clients]
            average_learners(disc_learners, self.global_learners_ensemble.domain_disc_learner,
                             weights=torch.ones((len(self.sampled_clients),)) / len(self.sampled_clients))
