...
def get_iid_class_and_labels(original_images, original_labels, N):
    ...
    for i in range(N):
        ...
        while (len(clients_labels[i]) < M):  
            sampled_class = torch.multinomial(p, 1)  
            if classes_by_index_len[sampled_class] == 0:
...
for i in range(client_number):
    # sampled_client = random_clients[i] #client index
    cifar_c, labels = [], []
    for img, label in zip(clients_images[i], clients_labels[i]):
        ...
        if 0 <= i <= 14:
            ...
            labels.append((label + 2) % C)
        cifar_c.append(np.uint8(convert_img(img)))
    clients_images[i] = cifar_c
    clients_labels[i] = labels
...
for i in range(client_number):
    ...
    # if 0 <= i <= 14:
    #     j = i
    #     save_data(client, './data/cifar10-c-60_client-simple2-iid-4concept-change-name/concept1_{}.pkl'.format(j))
    #
    # elif 14 < i <= 29:
    #     j = i-15
    #     save_data(client, './data/cifar10-c-60_client-simple2-iid-4concept-change-name/concept2_{}.pkl'.format(j))
    #
    # elif 29 < i <= 44:
    #     j = i-30
    #     save_data(client, './data/cifar10-c-60_client-simple2-iid-4concept-change-name/concept3_{}.pkl'.format(j))
    #
    # elif 44 < i <= 59:
    #     j = i-45
    #     save_data(client, './data/cifar10-c-60_client-simple2-iid-4concept-change-name/concept4_{}.pkl'.format(j))

    save_data(client,'./data/cifar100-c-60_client-simple2-iid-4concept-change-name-version2/{}.pkl'.format(i))
    # save_data(client,'./data/cifar100-c/{}.pkl'.format(i))


print('saving test...')

test_cifar_c, test_labels_1, test_labels_2, test_labels_3, test_labels_4 = [], [], [], [],[]
for img, label in zip(original_images_te, original_labels_te):
    ...
    if label < C:
        test_labels_4.append((label + 2) % C)
    else:
...
client_3 = {'images': np.uint8(np.array(test_cifar_c)),
              ...
              'type': 2}
client_4 = {'images': np.uint8(np.array(test_cifar_c)),
              'labels': np.uint8(np.array(test_labels_4)),
              'type': 3}

save_data(client_1,'./data/cifar100-c-60_client-simple2-iid-4concept-change-name-version2/test-1.pkl')
save_data(client_2,'./data/cifar100-c-60_client-simple2-iid-4concept-change-name-version2/test-2.pkl')
save_data(client_3,'./data/cifar100-c-60_client-simple2-iid-4concept-change-name-version2/test-3.pkl')
save_data(client_4,'./data/cifar100-c-60_client-simple2-iid-4concept-change-name-version2/test-4.pkl')
